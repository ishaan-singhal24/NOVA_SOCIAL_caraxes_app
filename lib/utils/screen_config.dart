...
class ScreenUtil {
  ...
  void init(BuildContext context) {
    width = ResponsiveWidget.isSmallScreen().not ? 1024 : 360;
    height = ResponsiveWidget.isSmallScreen().not ? 768 : 760;
    ...
    //rounding to multiple of 8
    int toRoundWidth = _screenWidth.toInt();
    int toRoundHeight = _screenHeight.toInt();
    int roundDownWidth = toRoundWidth - (toRoundWidth % 8);
    int roundUpWidth = toRoundWidth +
        toRoundWidth -
        1 -
        (toRoundWidth + toRoundWidth - 1) % toRoundWidth;
    int roundDownHeight = toRoundHeight - (toRoundHeight % 8);
    int roundUpHeight = toRoundHeight +
        toRoundHeight -
        1 -
        (toRoundHeight + toRoundHeight - 1) % toRoundHeight;
    width = (roundUpWidth - toRoundWidth) > (roundDownWidth - toRoundWidth)
        ? roundDownWidth
        : roundUpWidth;
    height = (roundUpHeight - toRoundHeight) > (roundDownHeight - toRoundHeight)
        ? roundDownHeight
        : roundUpHeight;
  }
