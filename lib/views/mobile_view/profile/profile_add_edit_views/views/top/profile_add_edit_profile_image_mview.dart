...
class ProfileAddEditProfileImageMView extends StatefulWidget {
  final ProfileMViewArgs args;

  const ProfileAddEditProfileImageMView({
    super.key,
    required this.args,
  });

  @override
  State<ProfileAddEditProfileImageMView> createState() =>
      _ProfileAddEditProfileImageMViewState();
}

class _ProfileAddEditProfileImageMViewState
    extends State<ProfileAddEditProfileImageMView> {
  bool _hasPickedImage = false;
  late ProfileNotifier _profileNotifier;
  Uint8List? _pickedProfilePicture;
  String? _encodedProfilePicture;
  ...
  Future<CroppedFile?> _pickImage() async {
    ...
    // return file;
    final profimg = await _cropImage(file?.path);
    return profimg;
  }

  Future<void> _onSubmit() async {
    if (_encodedProfilePicture?.isNotEmpty ?? false) {
      appLoadingMPopup(context);
      await _profileNotifier.uploadProfilePicture(
          base64File: _encodedProfilePicture!);
    }
                      ...
                      : CachedNetworkImage(
                          imageUrl:
                              _profileNotifier.userProfile?.profilePictureUrl ??
                                  AppAssets.nSampleUserProfilePic,
                          width: double.maxFinite,
                          height: double.maxFinite,
                          fit: BoxFit.cover,
                  ...
                  child: CustomButton(
                    ...
                    onPressed: () async {
                      if (_hasPickedImage) {
                        ...
                        // }
                        _pickedProfilePicture = await file?.readAsBytes();
                        List<int>? baseNonEncoded = _pickedProfilePicture;
                        final baseEncoded =
                            'data:$mimeType;base64,${base64Encode(baseNonEncoded ?? [])}';
                        _encodedProfilePicture = baseEncoded;
